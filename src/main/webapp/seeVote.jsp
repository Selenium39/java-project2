...
<script type="text/javascript">
	$(function() {
		...
		for(let i = 0; i < everyPageCount; i++) {//投票
			$("#btn" + voteIds[i]+"-seeVote").click(function() {
				$(location).attr("href","beforeVoteResult?id="+voteIds[i]);
			});
		}
		...
		$("#btn4").click(
				function() {//跳转上一页
					if (hasPrePage) {
						$(location).attr("href",
								"seeVote?currentPage=" + (currentPage - 1));
					} else {
						alert("没有上一页!!!");
					}
				});
		...
		$("#btn6").click(function() {//跳转尾页
			$(location).attr("href", "seeVote?currentPage=" + totalPage);
		});
	});
</script>
...
<body>
        <div>
        <table class="table">
     <caption> <span class="glyphicon glyphicon-user" aria-hidden="true"></span>所有投票</caption>
      <thead>
        <tr>
          <th>投票序号</th>
          <th>投票名</th>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>${vote.voteName}</td>
          <td><button class="btn btn-danger" id="btn${vote.voteId}-seeVote">查看投票</button></td>
        </tr>
        </c:forEach>
      </tbody>
    </table>
